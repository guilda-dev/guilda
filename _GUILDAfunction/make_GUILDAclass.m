...
function generate(~,~,edit_name,edit_discription,pop,lang,uifig)
    ...
    if contains(dirname,[config.pwd,filesep]+["_GUILDAsystem","_object","_Tutorial"])
        switch language
            case 'JPN'
    ...
    text_data = fileread([fullfile(config.pwd,'_GUILDAsystem',['@',pop.String{pop.Value}],'template',language),'.txt']);
