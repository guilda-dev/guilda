...
figure;
hold on
arrayfun(@(idx) plot(out.t,out.X{idx}(:,2),'LineWidth',1.5),1:16);
...
%シミュレーションの実行(制御器なしver)
    ...
    title('各同期発電機の周波数偏差(制御器なし)','FontSize',20)
    hold off
...
%シミュレーションの実行(制御器ありver)
    ...
    ylabel('周波数偏差','FontSize',15);
    legend({'機器2の周波数偏差','機器1の周波数偏差'})
    title('各同期発電機の周波数偏(制御器あり)','FontSize',20)
    hold off
...
%制御器を付ける前と後の比較のプロット
    ...
    legend({'機器2の周波数偏差(AGCなし)','機器1の周波数偏差(AGCなし)',...
                ...
                'Location','east')
    title('各同期発電機の周波数偏差(制御器あり/なしで比較)','FontSize',20)
    hold off
