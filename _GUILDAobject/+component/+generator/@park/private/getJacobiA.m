function A = getJacobiA(t, x, V, u, param, omega0) %#ok
    ...
    coeff_d = (Xd-Xdp);
    A(3,[1,3,6]) = [-coeff_d * (dId_dVq*dVq_ddel + alpha_d*df6_ddel), ...
                          -1 - coeff_d * (dId_dEq + alpha_d*df6_dEq), ...
                          -coeff_d * (dId_dpsid + alpha_d*df6_dpsid)];    

    alpha_q = (Xqp-Xqpp)/(Xqp-Xls)^2;
    coeff_q = (Xq-Xqp);
    A(4,[1,4,5]) = [coeff_q * (dIq_dVd*dVd_ddel + alpha_q*df5_ddel), ...
                         -1 + coeff_q * (dIq_dEd + alpha_q*df5_dEd), ...
                          coeff_q * (dIq_dpsiq + alpha_q*df5_dpsiq)];    

    A(5,[1,4,5]) = [df5_ddel, df5_dEd, df5_dpsiq];     
    A(6,[1,3,6]) = [df6_ddel, df6_dEq, df6_dpsid];     
