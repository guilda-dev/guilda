classdef one_axis < base_class.component
% モデル  ：同期発電機の一軸モデル
...
% 　出力　：componentクラスのインスタンス
    ...
    methods
        ...
        function set_avr(obj, avr)
            if isa(avr, 'avr')||isa(avr, 'component.generator.avr.base')
                obj.avr = avr;
            else
               error(''); 
            end
        ...
        function set_pss(obj, pss)
            if isa(pss, 'pss')||isa(pss, 'component.generator.pss.base')
                obj.pss = pss;
            else
                error('');
            end
        ...
        function set_governor(obj, governor)
            if isa(governor, 'governor')||isa(governor, 'component.generator.governor.base')
