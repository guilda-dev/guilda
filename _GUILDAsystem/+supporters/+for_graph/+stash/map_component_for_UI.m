classdef map_component_for_UI < supporters.for_graph.map_base
    ...
    methods
        function obj = map_component_for_UI(net,ax)
            obj@supporters.for_graph.map_base(net,ax)
            zlim(ax,[-15,15])
            view(ax,0,35)
            %hold(ax, 'off')

            obj.function_CompHeight     = @Height;
            obj.function_BusSize        = @(obj, V, I)     5;
            obj.function_CompSize       = @(obj,t,x,V,I,u) 10;
    ...
    methods(Access=private)
        ...
        function set_Marker(obj)
            for i = 1:obj.nbus
                ...
                elseif contains(nclass,'GFM') || contains(nclass,'gfm')
                    obj.Graph.NodeLabel{obj.nbus+i}  = [' GFM',num2str(i)];
