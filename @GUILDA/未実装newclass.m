function newclass()
% Create a new class (GUI launches)
...
function generate(~,~,edit_name,edit_discription,pop,lang,uifig)
    ...
    if contains(dirname,[GUILDA.pwd,filesep]+["_GUILDAsystem","_object","_GUILDAtutorial"])
        switch language
            case 'JPN'
                error(['GUILDAのソースフォルダ下以外を選択してください',newline,'ソースフォルダ',newline,'./_GUILDAsytem/',newline,'./_object/',newline,'./_GUILDAtutorial/'])
                % errordlg({'GUILDAのソースフォルダ下以外を選択してください','./_GUILDAsytem/','./_object/','./_GUILDAtutorial/'})
            case 'ENG'
                error(['Select a path not under the GUILDA source folder',newline,'Source Folder',newline,'./_GUILDAsytem/',newline,'./_object/',newline,'./_GUILDAtutorial/'])
                % errordlg({'Select a path not under the GUILDA source folder','./_GUILDAsytem/','./_object/','./_GUILDAtutorial/'})
        end
    end
    ...
    text_data = fileread([fullfile(GUILDA.pwd,'_GUILDAsystem',['@',pop.String{pop.Value}],'template',language),'.txt']);
