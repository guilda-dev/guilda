    ...
    methods
        ...
        function obj = Inverter(para)
            % Set parameters
                if nargin==0
                    ...
                    % parameter = [0,3.67*1e-5,1.91*1e-4,6.1*1e-4,0,1];
                    
                    %3busTutorialに合わせるためのパラメータ設定
                    parameter = [0,0.0034, 0.0202,0.0059,0,1];
                    para = array2table(parameter,"VariableNames",{'R','L','C','L_g','R_g','n'});
            ...
            %}
            %{
                obj.set_Sbase(100*1e6)        
                obj.set_Vbase( 230*1e3    );
            %}
            % Set default class
        ...
        % Methods to define the state, number of input ports, and variable names
            function nx = get_nx(obj)
                ...
                nx = 4 + obj.vsc_controller.get_nx()  ...
        ...
        function [dx, constraint] = get_dx_constraint(obj, t, x, V, I, u)
                ...
                x_con = x( (1:con.get_nx) + 4                           );
                x_ref = x( (1:ref.get_nx) + 4 + con.get_nx              );
                x_dc  = x( (1:dc.get_nx ) + 4 + con.get_nx + ref.get_nx );
                ...
                % Get terminal voltage values from the reference model.
                    vdc = dc.get_vdc( x_dc, u_dc);
                    [delta,domega,vdq_hat]  = ref.get_Vterminal(x_ref,V,I,u_ref,vdc);
                ...
                % Calculate dx
                    ...
                    ix = (1/2) * m.' * is_dq;
                    [dx_dc,  vdc    ] =  dc.get_dx_vdc( t, x_dc , u_dc , v_dq, I_dq, ix, V, I);
    
                    % Calculate references model dynamics
                    [dx_ref, con_ref] = ref.get_dx_constraint(t, x_ref, V, I, u_ref, vdc); %#ok
                    ...
                    %{
                       hold on
                       plot(t, d_isdq,'o',"Color",'g');
                       plot(t, d_vdq, 'o','Color','b');
                       plot(t, d_idq, 'o', 'Color','r');
                    %}
                    %
                    d_isdq= (- R * eye(2) * is_dq - v_dq + vs_dq) / L;
                    d_vdq = ( is_dq - I_dq) / C; % d_vdq = (is_dq - i_dq) / p.C;
                    % d_idq = (- R_g * eye(2) * i_dq - V_dq + v_dq) / L_g;
                    %}
                ...
                dx  = [d_isdq; d_vdq; dx_con; dx_ref; dx_dc];
                constraint = con_V; %[con_ref;con_I];
            ...
            function [x_st,u_st] = get_equilibrium(obj, Vc, Ic, flag) %#ok
                ...
                % Calculate equilibrium of "Vdq,Idq"
                    vdc_st = obj.dc_source.parameter.vdc_st/ obj.Vbase;
                    [delta_st, domega_st, Vdq] = obj.reference_model.get_Vterminal(x_ref,V,I,u_ref,vdc_st);
                ...
                % Converter
                    ...
                    R_g = p.R_g / obj.Zbase;
                    L_g = p.L_g / obj.Lbase;
                    % grid_tensor = [0  L_g*omega_st;
                                % -L_g*omega_st 0];
                    % V_dq = tensor * V;


                    vdq_st  = Vdq;% + grid_tensor.'*Idq;%+(p.R_g*eye(2) + omega_st*p.L_g*[0,-1;1,0]) * idq_st;
                    isdq_st = Idq;% + omega_st*C*[0,-1;1,0]*vdq_st;
                    % idq_st = Idq;
                ...
                x_st = [isdq_st; vdq_st; x_con; x_ref; x_dc];
