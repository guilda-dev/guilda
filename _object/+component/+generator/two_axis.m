...
%　引数　: parameter : table型．「'Xd', 'Xd_p','Xq','Xq_p','Td_p','Tq_p','M','D'」を列名として定義
    ...
    methods
        ...
        function [dx, con] = get_dx_constraint(obj, t, x, V, I, u)%#ok
            ...
            % パラメータを追加
            p = obj.parameter{:,{'Xd','Xd_p','Xq','Xq_p','Td_p','Tq_p','M','D'}};
            Xd   = p(1);
            Xdp  = p(2);
            Xq   = p(3);
            Xqp  = p(4);
            ...
            % 状態の抽出
            [delta,omega,Edq] = obj.get_Vterminal(x_gen);
            ...
            Efq = Edq(1) - (Xq-Xqp)*Idq(2);
            
            P  = V.'*I;

            % Efq = Xq*Edq(1)/Xqp - (Xq/Xqp-1)*Vdq(1)
            % = 1/Xqp (Xqp*Ed + (Xq-Xqp)*Ed - (Xq-Xqp)*Vd )
            % = 1/Xqp (Xqp*Ed + (Xq-Xqp)*(Ed-Vd) )
            ...
            % 微分項の計算
                ...
                dx = [ddelta; domega; dEq; dEd; dx_sub];
